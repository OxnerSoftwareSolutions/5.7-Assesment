...
function userHasVisitedParkOnWishlist(users, username1, username2) {
  const user1 = users[username1].visited;//object shorthand for user1 variable that represents an array of usernames that have visited the park assigned to them.
  const user2 = users[username2].wishlist;//object shorthand for user2 variable that represents the wishlishts
...
function getUsersForUserWishlist(users, username) {
  ...
  for (let username in users) {//loop through the array of user
    if (userWishlist.some((id) => users[username].visited.includes(id))) {
     usersForUserWishlist.push(username); 
    }
